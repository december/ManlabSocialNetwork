...
def GetLog(r, p, u, t, tau, c, d): #root_tweet, parent_tweet, parent_user, parent_time, tau, cascade log, depth
	...
	for f in edgemap[u]:
		...
		thres = x[edgemap[u][f]] ** -(d - 1) * pi[edgemap[u][f]] * GetPhi(phi1, phi2, phi3, phi4, phi5, tau, f) * GetPhi(phi1_post, phi2_post, phi3_post, phi4_post, phi5_post, tau, u)
...
for j in range(sims):
	...
	for i in range(users):
		...
		while ts < te:
			...
			temp.append(-1)
			cascade.append(temp)
			tau = GetTau(phi1_post, phi2_post, phi3_post, phi4_post, phi5_post, newi)
