...
def LnLc(omega, pi, x, philist, c, tau): #ln fromulation of one cascades's likelihood on tau(do not include part of Q)
	...
	omega_rc = tf.gather(omega, rc[:, 3], axis=0)
	pi_rc = tf.gather(pi, rc[:, 0], axis=0)
	x_rc = tf.gather(x, rc[:, 0], axis=0)
	phi_rc = tf.gather(philist[tau], rc[:, 3], axis=0)
	s += tf.reduce_sum(tf.log(omega_rc) - omega_rc * rc[:, 1] + tf.log(pi_rc) - rc[:, 2] * tf.log(x_rc) + tf.log(phi_rc))

	omega_nc = tf.gather(omega, nc[:, 3], axis=0)
	pi_nc = tf.gather(pi, nc[:, 0], axis=0)
	x_nc = tf.gather(x, nc[:, 0], axis=0)
	phi_nc = tf.gather(philist[tau], nc[:, 3], axis=0)
...
rusc[:, 0] = tf.cast(rusc[:, 0], dtype=tf.int64)
rusc[:, 3] = tf.cast(rusc[:, 3], dtype=tf.int64)
nrusc[:, 0] = tf.cast(nrusc[:, 0], dtype=tf.int64)
nrusc[:, 3] = tf.cast(nrusc[:, 3], dtype=tf.int64)
for key in rusc_dic:
