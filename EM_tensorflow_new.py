...
def LnLc(omega, pi, x, philist, c, tau): #ln fromulation of one cascades's likelihood on tau(do not include part of Q)
	...
	omega_rc = tf.gather(omega, tf.cast(rc[:, 3], dtype=int64), axis=0)
	pi_rc = tf.gather(pi, tf.cast(rc[:, 0], dtype=int64), axis=0)
	x_rc = tf.gather(x, tf.cast(rc[:, 0], dtype=int64), axis=0)
	phi_rc = tf.gather(philist[tau], tf.cast(rc[:, 3], dtype=int64), axis=0)
	s += tf.reduce_sum(tf.log(omega_rc) - omega_rc * rc[:, 1] + tf.log(pi_rc) - rc[:, 2] * tf.log(x_rc) + tf.log(phi_rc))

	omega_nc = tf.gather(omega, tf.cast(nc[:, 3], dtype=int64), axis=0)
	pi_nc = tf.gather(pi, tf.cast(nc[:, 0], dtype=int64), axis=0)
	x_nc = tf.gather(x, tf.cast(nc[:, 0], dtype=int64), axis=0)
	phi_nc = tf.gather(philist[tau], tf.cast(nc[:, 3], dtype=int64), axis=0)
