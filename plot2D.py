...
# 设置最大的范围，保证画图的坐标轴一致性
REAL_MAX = {'size': 120, 'depth': 20, 'width': 60, 'wiener': 10, 'diameter': 23}

def showtime(s=''):
    print time.strftime('time: %Y-%m-%d %H:%M:%S', time.localtime(time.time())) + ' ' + str(s)


col_names = ['cascade_id', 'size', 'depth', 'width', 'wiener', 'diameter']
...
def plot_by_x_y(x_list, y_list, xy_f_name):
    ...
    # ylim(x_min, x_max)
    # xlim(y_min, y_max)
    ylim(x_min, REAL_MAX[x_name],'fontsize'=20)
    xlim(y_min, REAL_MAX[y_name],'fontsize'=20)
    xlabel(y_name)
    ylabel(x_name)
    # show()
